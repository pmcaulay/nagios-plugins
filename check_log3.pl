...
#
# Last updated 2013-10-11 by Peter Mc Aulay <peter@zeron.be>
#
...
if ($log_pattern) {
	...
	undef $seek_file unless $seek_file eq '/dev/null';
}
...
if (not $seek_file) {
	...
	$basename =~ s/^.*\///g;
	$seek_file = "$tmpdir/" . $basename . ".seek";
}
...
-m, --logfile-pattern=<expression>
    ...
    either a directory or to /dev/null.
